...
from src.search.query_processor import QueryProcessor, preprocess_query
import logging
...
def _execute_search(
    ...
    # Fuse results
    return _rrf_fuse(lex_hits, knn_hits, window_size, settings.top_k)
...
def _interactive_loop(
    ...
    while True:
        ...
        if not skip_correction:
            corrected_query, corrections, confidence = query_processor.process_query(query)
        ...
        hits = _execute_search(client, embedder, query_to_search)
...
def main() -> None:
    ...
    custom_vocab = [
        ...
        'emnlp', 'naacl', 'eacl', 'coling', 'lrec', 'conll', 'tacl',
        # Add more technical terms as needed
        'roberta', 'xlnet', 'albert', 'electra', 't5', 'bart',
        'squad', 'glue', 'superglue', 'bleu', 'rouge', 'meteor',
    ]
    query_processor = QueryProcessor(custom_vocabulary=custom_vocab)
    
    # Run interactive loop
    _interactive_loop(client, embedder, query_processor)
